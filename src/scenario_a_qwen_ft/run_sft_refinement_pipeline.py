...
from scenario_a_qwen_ft.sft_refinement_pipeline import QwenSFTRefinementBuilder


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run Scenario A Qwen SFT refinement v2.")
...
def main() -> None:
    ...
    builder = QwenSFTRefinementBuilder(
        project_root=project_root,
        question_root=(args.question_root or (project_root / "rag_outputs" / "qwen_ft_instruction_a")).resolve(),
        prior_output_root=(args.prior_output_root or (project_root / "rag_outputs" / "qwen_ft_instruction_expansion_v1")).resolve(),
        output_root=(args.output_root or (project_root / "rag_outputs" / "qwen_ft_instruction_expansion_v2_refine")).resolve(),
