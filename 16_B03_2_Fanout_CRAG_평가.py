...
DEFAULT_EVAL_DIR = OUTPUT_DIR / "b03_2_fanout_history_crag_eval"
...
def source_name_tokens(source_file_name: str) -> set[str]:
    tokens = {
        token
        for token in re.findall(r"[0-9A-Za-z가-힣]+", str(source_file_name or "").lower())
        if len(token) >= 2
    }
    stopwords = {
        "pdf",
...
def select_fanout_sources(
    ...
    query_tokens = {
        token
        for token in re.findall(r"[0-9A-Za-z가-힣]+", f"{question} {retrieval_query}".lower())
        if len(token) >= 2
    }
    scored: list[tuple[float, int, str]] = []
    for index, source_file in enumerate(prior_source_files):
        source_tokens = source_name_tokens(source_file)
        overlap = len(query_tokens & source_tokens)
        partial = 0.0
        source_value = str(source_file or "").lower()
        for token in query_tokens:
            if token and token in source_value:
                partial += 0.25
        scored.append((overlap + partial, -index, source_file))

    scored.sort(key=lambda item: (item[0], item[1]), reverse=True)
    selected = [source for score, _, source in scored if score > 0][:max_sources]
    if not selected:
        selected = prior_source_files[:max_sources]
    return selected
...
def vector_search_filtered(
    ...
    try:
        ...
        return [
            row
            for row in vector_search(collection, query_embedding, max(candidate_k * 4, 20))
            if str(row["metadata"].get("source_file_name", "")) == str(source_file_name)
        ][:candidate_k]
...
def bm25_search_filtered(
    ...
    rows: list[dict[str, Any]] = []
    rank = 1
    for row_index, score in ranked_pairs:
        row = chunk_rows[row_index]
        if str(row.get("source_file_name", "")) != str(source_file_name):
            continue
        rows.append(
            {
                "rank": rank,
...
def build_source_fanout_candidates(
    collection,
    bm25_index_payload: dict[str, Any],
    query_embedding: list[float],
    query: str,
    ...
    selected: list[dict[str, Any]] = []
    for source_file in fanout_sources:
        vector_rows = vector_search_filtered(collection, query_embedding, per_source_k * 2, source_file)
        bm25_rows = bm25_search_filtered(bm25_index_payload, query, per_source_k * 2, source_file)
        if not vector_rows and not bm25_rows:
            continue
        fused_rows = fuse_rrf(vector_rows, bm25_rows, vector_weight, bm25_weight, rrf_k)
        reranked_rows = rerank_with_profile(fused_rows, profile, question)
        for row in reranked_rows[:per_source_k]:
            new_row = dict(row)
            new_row["fanout_source"] = source_file
            selected.append(new_row)
    return unique_by_chunk_id(selected)


def select_followup_fanout_candidates(
    candidates: list[dict[str, Any]],
    fanout_sources: list[str],
    top_k: int,
) -> list[dict[str, Any]]:
    deduped = unique_by_chunk_id(candidates)
    ...
    for source_file in fanout_sources:
        for row in deduped:
            if str(row["metadata"].get("source_file_name", "")) != str(source_file):
                continue
            chunk_id = str(row.get("chunk_id", ""))
...
def main() -> None:
    ...
    parser.add_argument("--fanout-max-sources", dest="fanout_max_sources", type=int, default=2)
    parser.add_argument("--fanout-per-source", dest="fanout_per_source", type=int, default=1)
    ...
    for row in questions:
        ...
        fanout_sources: list[str] = []
        fanout_mode = False
        if profile["follow_up"] and len(prior_source_files) >= 2:
            fanout_sources = select_fanout_sources(
                question=row["question"],
                retrieval_query=retrieval_query,
                prior_source_files=prior_source_files,
                max_sources=args.fanout_max_sources,
            )
            fanout_mode = len(fanout_sources) >= 2
        ...
        if fanout_mode:
            fanout_initial = build_source_fanout_candidates(
                collection=collection,
                bm25_index_payload=bm25_index_payload,
                query_embedding=query_embedding,
                query=retrieval_query,
                fanout_sources=fanout_sources,
                per_source_k=args.fanout_per_source,
            ...
            initial_pool = rerank_with_profile(unique_by_chunk_id(fanout_initial + initial_ranked), profile, row["question"])
            if profile["follow_up"] and prior_source_files:
                initial_pool = apply_source_bias(initial_pool, prior_source_files, args.history_bias)
            evaluator_candidates = select_followup_fanout_candidates(initial_pool, fanout_sources, args.judge_candidate_k)
        else:
        ...
        if crag_result["judgment"] == "INCORRECT":
            ...
            if crag_result["need_second_pass"]:
                ...
                if fanout_mode:
                    second_fanout = build_source_fanout_candidates(
                        collection=collection,
                        bm25_index_payload=bm25_index_payload,
                        query_embedding=second_embedding,
                        query=second_pass_query,
                        fanout_sources=fanout_sources,
                        per_source_k=args.fanout_per_source,
                        vector_weight=args.second_vector_weight,
                        bm25_weight=args.second_bm25_weight,
                        rrf_k=args.rrf_k,
                        profile=profile,
                        question=row["question"],
                    )
                    ...
                    second_fanout = []
                merged = unique_by_chunk_id(refined + second_fanout + second_ranked + initial_ranked)
            ...
            if fanout_mode:
                final_candidates = select_followup_fanout_candidates(candidate_pool_for_answer, fanout_sources, final_k)
            elif profile["follow_up"]:
        ...
        result_rows.append(
            {
                ...
                "fanout_mode": int(fanout_mode),
                "fanout_sources": " | ".join(fanout_sources),
                ...
                "retrieval_mode": "hybrid_contextual_history_crag_fanout",
                "vector_weight": args.vector_weight,
                "bm25_weight": args.bm25_weight,
                ...
                preview_rows.append(
                    {
                        ...
                        "fanout_source": item.get("fanout_source", ""),
                    }
                )
        ...
        json.dumps(
            {
                ...
                "fanout_max_sources": args.fanout_max_sources,
                "fanout_per_source": args.fanout_per_source,
                "candidate_k": args.candidate_k,
